...
#for locality_aware_nms and lanms
...
def reader3(boxes,orig,crop_ratio):
    '''
    extract text from given region 
    :param boxes: boxes detected by EAST
    :param orig: orginal image
    :param crop_ratio: list to determine crop ratio (data given under cell location_ratio1)
    ...
    for box in boxes:
        ...
        _crp = _location_ratio(h, w, crop_ratio, orig, points[0][0][0], points[0][0][1])
        if(_crp == True):
            
            _roi = roi(orig,box)
            _roi = preprocess2(_roi)
            config = ("--oem 1  --psm 7")
            text.append(pytesseract.image_to_string(_roi, config=config))
        
    
    return text
#%% location_ratio1 (current)
"""
        "crop" speaker name       : [0.7, 0.97 , 0.000001 ,0.2] ,
        "main_screen"    : [0.135, 0.95 , 0.095 , 0.84]
"""
def _location_ratio(he,wi,crop,image,x_loc,y_loc) :
    '''
    determine if box is in parameters
    :param he: height of orignal image
    :param wi: width of orignal image
    :param crop: crop ratio (data given above)
    :param image: orignal image
    :param x_loc: start x coordinate of box
    :param y_loc: start y coordinate of box
    :return: boolean if box within parameters
    '''
    x1= float(crop[0])
    x2= float(crop[1])
    y1= float(crop[2])
    ...
    # crp = cropped image
    #_crp = image[start_row:end_row , start_col:end_col]
    # return cropped image
    
    if( ( x_loc > start_row and x_loc < end_row ) and ( y_loc > start_col and y_loc < end_col  ) ):
        return True
    else:
        return False

def _location_ratio2(he,wi,crp_ratio,img):
    
    x1= float(crp_ratio[0])
    x2= float(crp_ratio[1])
    y1= float(crp_ratio[2])
    y2= float(crp_ratio[3])
    
    # dimesions to crop
    start_row , start_col = int(he*x1) , int(wi*y1)
    end_row , end_col = int(he*x2) , int(wi*y2)
    # crp = cropped image
    _crp = img[start_row:end_row , start_col:end_col]
    # return cropped image
    points = [[start_row,start_col], [start_row,end_col] , [end_row , end_col] , [end_row,start_col]]
...
#    cv2.drawContours(mask, [points], -1, (255, 255, 255), -1, cv2.LINE_AA)
    cv2.rectangle(mask, points[0],points[2],(255,0,0) , 1)
...
def _box_validator(boxes):
    '''
     removes overlapping and invalid boxes
    :param boxes: boxes returned by EAST
    :return: valid and non-overlapping boxes
    '''
    valid_box = []
...
def roi(img,box):
    '''
     crops according to boxes 
    :param img: orginal image
    :param box: box containing points for region of interest
...
#   cropped = res[rect2[1]: rect2[1] + rect2[3], rect2[0]: rect2[0] + rect2[2]]   
    ...
    #tesseract(LSTM) best detects with 30pixel height of Captial letter
...
def expand2(points, margin):
    ...
    points[1][0][0] = points[1][0][0] + margin*2.4
    points[1][0][1] = points[1][0][1] - margin*2.4
    points[2][0][0] = points[2][0][0] + margin*2.4
    points[2][0][1] = points[2][0][1] + margin*4.4
    points[3][0][0] = points[3][0][0] - margin*2.4
    points[3][0][1] = points[3][0][1] + margin*4.4
    return points
"""
    points[0][0][0] = points[0][0][0] - margin 
    points[0][0][1] = points[0][0][1] - margin
...
#%% preprocess2.0 (current)
def preprocess2(img): 
    #tesseract detects better with RGB
...
def data_extractor(boxes,image):
    '''
    Extract speakers name and main content into 2 seprate lists
    :param boxes: boxes returned by EAST
    :param image: orignal image
    '''
    global speaker_name
    ...
    crp_r = {
                "speaker"        : [0.7, 0.97 , 0.000001 ,0.2] ,
                "main_screen"    : [0.135, 0.95 , 0.095 , 0.84]
            }
    speaker_name = reader3(boxes, image, crp_r["speaker"])
    print(speaker_name)
    content = reader3(boxes,image,crp_r["main_screen"])
    print(content)
